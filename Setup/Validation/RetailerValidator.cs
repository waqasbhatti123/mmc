...
{
    public class RetailerValidator : AbstractValidator<RetailerData>
    {
        public RetailerValidator()
        {
            ...
            RuleFor(RH => RH.Phone1).Must(BeUniquePhone2).WithMessage("Phone2 Already Exist");
        }
        ...
        private bool BeUniquePhone1(string strPhone1)
        {
            ...
            if (!String.IsNullOrEmpty(strPhone1))
            {
                ...
                {
                    if (dbContext.Retailers.FirstOrDefault(x => x.Phone1 == strPhone1) == null) return true;
                    boolFlag = false;
                }
            }
            return boolFlag;
        ...
        private bool BeUniquePhone2(string strPhone2)
        {
            ...
            if (!String.IsNullOrEmpty(strPhone2))
            {
                using (FOSDataModel dbContext = new FOSDataModel())
                {
                    if (dbContext.Retailers.FirstOrDefault(x => x.Phone2 == strPhone2) == null) return true;
                    boolFlag = false;
                }
            }
            return boolFlag;
