...
{
    public class RegionalHeadValidator : AbstractValidator<RegionalHeadData>
    {
        public RegionalHeadValidator()
        {
        ...
        private bool BeUniquePhone1(string strPhone1)
        {
            ...
            if (!String.IsNullOrEmpty(strPhone1))
            {
                ...
                {
                    if (dbContext.RegionalHeads.FirstOrDefault(x => x.Phone1 == strPhone1) == null) return true;
                    boolFlag = false;
                }
            }
            return boolFlag;
        ...
        private bool BeUniquePhone2(string strPhone2)
        {
            ...
            if (!String.IsNullOrEmpty(strPhone2))
            {
                using (FOSDataModel dbContext = new FOSDataModel())
                {
                    if (dbContext.RegionalHeads.FirstOrDefault(x => x.Phone2 == strPhone2) == null) return true;
                    boolFlag = false;
                }
            }
            return boolFlag;
